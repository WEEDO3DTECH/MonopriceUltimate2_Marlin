...
uint8_t wtvar_sdcard_filelist[SDCARD_MENU_PAGE_ITEMS][20];
...
void wt_restart() { asm volatile ("jmp 0"); };				// restart
void wt_saveoff();
void wt_zoffset_prepare_function();
void wt_filament_setting_menu();
void wt_door_setting_menu();
void wt_levelbed_begin();
void wt_levelbed_moving();
void wt_load_sdcard();
void wt_sdcard_showpage();
void wt_Load_door_setting_menu();
void wt_Load_filament_setting_menu();
void wtlcd_ChoiceZMaxMenu();
void wtlcd_LoadZMaxMenu();
...
#endif
	//lcd_reset_status();
	lcd_setstatus(card.longest_filename(), true);
	lcd_return_to_status();
...
void lcd_main_menu() 
{
	...
	MENU_BACK_MULTI(MMSG_WATCH[wtvar_language]);
	...
	if (card.cardOK) 
	{
		if (card.isFileOpen())
		{
			...
			else
			{
				...
				MENU_ITEM_MULTI(gcode, MMSG_FILAMENTIN[wtvar_language], PSTR("M104 T0 S210\r\nM701 T0"));
...
void lcd_babystep_zoffset()
{
	...
	if (lcdDrawUpdate)
	{
		lcd_implementation_drawedit(MMSG_ZPROBE_TITLE[wtvar_language], ftostr43sign(zprobe_zoffset));
...
void lcd_tune_menu() {
	...
	// Speed:
	//
	MENU_ITEM_EDIT_MULTI(int3, MMSG_SPEED[wtvar_language], &feedrate_percentage, 10, 999);
	...
	//
#if HOTENDS == 1
	MENU_MULTIPLIER_ITEM_EDIT_CALLBACK_MULTI(int3, MMSG_NOZZLE[wtvar_language], &thermalManager.target_temperature[0], 0, HEATER_0_MAXTEMP - 15, watch_temp_callback_E0);
#else // HOTENDS > 1
	MENU_MULTIPLIER_ITEM_EDIT_CALLBACK_MULTI(int3, MMSG_NOZZLE[wtvar_language] MSG_N1, &thermalManager.target_temperature[0], 0, HEATER_0_MAXTEMP - 15, watch_temp_callback_E0);
	MENU_MULTIPLIER_ITEM_EDIT_CALLBACK_MULTI(int3, MMSG_NOZZLE[wtvar_language] MSG_N2, &thermalManager.target_temperature[1], 0, HEATER_1_MAXTEMP - 15, watch_temp_callback_E1);
#if HOTENDS > 2
	MENU_MULTIPLIER_ITEM_EDIT_CALLBACK_MULTI(int3, MMSG_NOZZLE[wtvar_language] MSG_N3, &thermalManager.target_temperature[2], 0, HEATER_2_MAXTEMP - 15, watch_temp_callback_E2);
#if HOTENDS > 3
	MENU_MULTIPLIER_ITEM_EDIT_CALLBACK_MULTI(int3, MMSG_NOZZLE[wtvar_language] MSG_N4, &thermalManager.target_temperature[3], 0, HEATER_3_MAXTEMP - 15, watch_temp_callback_E3);
#if HOTENDS > 4
	MENU_MULTIPLIER_ITEM_EDIT_CALLBACK_MULTI(int3, MMSG_NOZZLE[wtvar_language] MSG_N5, &thermalManager.target_temperature[4], 0, HEATER_4_MAXTEMP - 15, watch_temp_callback_E4);
#endif // HOTENDS > 4
#endif // HOTENDS > 3
#endif // HOTENDS > 2
#endif // HOTENDS > 1
	...
	//
#if HAS_HEATED_BED
	MENU_MULTIPLIER_ITEM_EDIT_CALLBACK_MULTI(int3, MMSG_BED[wtvar_language], &thermalManager.target_temperature_bed, 0, BED_MAXTEMP - 15, watch_temp_callback_bed);
#endif
...
#if FAN_COUNT > 0
#if HAS_FAN0
	MENU_MULTIPLIER_ITEM_EDIT_MULTI(int3, MMSG_FAN_SPEED[wtvar_language], &fanSpeeds[0], 0, 255);
#if ENABLED(EXTRA_FAN_SPEED)
	MENU_MULTIPLIER_ITEM_EDIT(int3, MSG_EXTRA_FAN_SPEED FAN_SPEED_1_SUFFIX, &new_fanSpeeds[0], 3, 255);
#endif
#endif
#if HAS_FAN1
	MENU_MULTIPLIER_ITEM_EDIT_MULTI(int3, MMSG_FAN_SPEED[wtvar_language] " 2", &fanSpeeds[1], 0, 255);
#if ENABLED(EXTRA_FAN_SPEED)
	MENU_MULTIPLIER_ITEM_EDIT(int3, MSG_EXTRA_FAN_SPEED " 2", &new_fanSpeeds[1], 3, 255);
#endif
#endif
#if HAS_FAN2
	MENU_MULTIPLIER_ITEM_EDIT_MULTI(int3, MMSG_FAN_SPEED[wtvar_language] " 3", &fanSpeeds[2], 0, 255);
#if ENABLED(EXTRA_FAN_SPEED)
	...
	//
#if EXTRUDERS == 1
	MENU_ITEM_EDIT_CALLBACK_MULTI(int3, MMSG_FLOW[wtvar_language], &planner.flow_percentage[0], 10, 999, _lcd_refresh_e_factor_0);
#else // EXTRUDERS > 1
	MENU_ITEM_EDIT_CALLBACK_MULTI(int3, MMSG_FLOW[wtvar_language], &planner.flow_percentage[active_extruder], 10, 999, _lcd_refresh_e_factor);
	MENU_ITEM_EDIT_CALLBACK_MULTI(int3, MSG_MMSG_FLOW[wtvar_language]FLOW MSG_N1, &planner.flow_percentage[0], 10, 999, _lcd_refresh_e_factor_0);
	MENU_ITEM_EDIT_CALLBACK_MULTI(int3, MMSG_FLOW[wtvar_language] MSG_N2, &planner.flow_percentage[1], 10, 999, _lcd_refresh_e_factor_1);
#if EXTRUDERS > 2
	MENU_ITEM_EDIT_CALLBACK_MULTI(int3, MMSG_FLOW[wtvar_language] MSG_N3, &planner.flow_percentage[2], 10, 999, _lcd_refresh_e_factor_2);
#if EXTRUDERS > 3
	MENU_ITEM_EDIT_CALLBACK_MULTI(int3, MMSG_FLOW[wtvar_language] MSG_N4, &planner.flow_percentage[3], 10, 999, _lcd_refresh_e_factor_3);
#if EXTRUDERS > 4
	MENU_ITEM_EDIT_CALLBACK_MULTI(int3, MMSG_FLOW[wtvar_language] MSG_N5, &planner.flow_percentage[4], 10, 999, _lcd_refresh_e_factor_4);
#endif // EXTRUDERS > 4
#endif // EXTRUDERS > 3
#endif // EXTRUDERS > 2
#endif // EXTRUDERS > 1
	...
	MENU_ITEM_MULTI(submenu, MMSG_FILAMENT_SENSOR[wtvar_language], wt_Load_filament_setting_menu);

	MENU_ITEM_MULTI(function, MMSG_DOOR_SENSOR[wtvar_language], wt_Load_door_setting_menu);
#endif
...
void _lcd_goto_next_corner() {
	line_to_z(4.0 + 170 - wtvar_Z_MAX_POS);
	switch (bed_corner) {
	case 0:
		current_position[X_AXIS] =175;
		current_position[Y_AXIS] = 148;
		break;
	case 1:
	...
	line_to_z(0.0 + 170 - wtvar_Z_MAX_POS);
	if (++bed_corner > 3
#if ENABLED(LEVEL_CENTER_TOO)
		+ 1
#endif
...
void _lcd_corner_submenu() {
	START_MENU();
	MENU_ITEM_MULTI(function, MMSG_WELCOME_CONINUE[wtvar_language], _lcd_goto_next_corner);
	MENU_ITEM_MULTI(function, MMSG_WELCOME_SKIP[wtvar_language], lcd_goto_previous_menu_no_defer);
	END_MENU();
}
...
void lcd_bed_leveling() {
	...
	MENU_ITEM_MULTI(gcode, MMSG_LEVEL_BED[wtvar_language], is_homed ? PSTR("G29") : PSTR("G28\nG29"));
	...
	MENU_ITEM_MULTI(submenu, MMSG_ZPROBE_ZOFFSET[wtvar_language], lcd_babystep_zoffset);
#elif HAS_BED_PROBE
	MENU_ITEM_MULTI(float52, MMSG_ZPROBE_ZOFFSET[wtvar_language], &zprobe_zoffset, Z_PROBE_OFFSET_RANGE_MIN, Z_PROBE_OFFSET_RANGE_MAX);
#endif
...
void lcd_prepare_menu() {
	...
	// Auto Home
	//
	MENU_ITEM_MULTI(gcode, MMSG_AUTO_HOME[wtvar_language], PSTR("G28 Z F500\r\nG28 X Y F1000"));

	//
	// Level Bed
	MENU_ITEM_MULTI(function, MMSG_LEVEL_BED[wtvar_language], wtlcd_LoadZMaxMenu);
...
#endif
	manual_move_start_time = millis() + (move_menu_scale < 3.99f ? 0UL : 250UL); 
...
void lcd_control_menu()
{
...
#ifdef EnableRunoutAndDoor
	MENU_ITEM_MULTI(function, MMSG_FILAMENT_SENSOR[wtvar_language], wt_Load_filament_setting_menu);
	...
	MENU_ITEM_MULTI(function, MMSG_RESTORE_DEFAULT[wtvar_language], lcd_factory_settings);
...
#endif
	START_MENU();
	MENU_BACK_MULTI(MMSG_TEMPERATURE[wtvar_language]);
	MENU_ITEM_EDIT_MULTI(int3, MMSG_FAN_SPEED[wtvar_language], &lcd_preheat_fan_speed[material], 0, 255);
#if HAS_TEMP_HOTEND
	MENU_ITEM_EDIT_MULTI(int3, MMSG_NOZZLE[wtvar_language], &lcd_preheat_hotend_temp[material], MINTEMP_ALL, MAXTEMP_ALL - 15);
#endif
#if HAS_HEATED_BED
	MENU_ITEM_EDIT_MULTI(int3, MMSG_BED[wtvar_language], &lcd_preheat_bed_temp[material], BED_MINTEMP, BED_MAXTEMP - 15);
#endif
#if ENABLED(EEPROM_SETTINGS)
	MENU_ITEM(function, MSG_STORE_EEPROM, lcd_store_settings);
#endif
...
void lcd_sdcard_menu()
{
	...
	// debug
	//SERIAL_ECHOLNPAIR("file count %d", fileCnt);
	...
	for (uint16_t i = 0; i < fileCnt; i++) {
		if (_menuLineNr == _thisItemNr) {
			...
			if (card.filenameIsDir)
				MENU_ITEM_MULTI(sddirectory, MMSG_CARD_MENU[wtvar_language], card);
			else
				MENU_ITEM_MULTI(sdfile, MMSG_CARD_MENU[wtvar_language], card);
		}
		else {
			MENU_ITEM_DUMMY();
		}
...
static const char* advanced_pause_header() {
	...
	case ADVANCED_PAUSE_MODE_LOAD_FILAMENT:
		return MMSG_FILAMENTIN[wtvar_language];
	case ADVANCED_PAUSE_MODE_UNLOAD_FILAMENT:
...
void menu_action_listfile(const char* filename)
{
	card.getfilename(encoderLine - 1 + wtvar_sdcard_pageid * SDCARD_MENU_PAGE_ITEMS);
...
{
	START_MENU();
	STATIC_ITEM_MULTI(MMSG_WELCOME_SCREEN_1[wtvar_language], false, false);

	MENU_ITEM_MULTI(function, MMSG_WELCOME_CONINUE[wtvar_language], wtlcd_goto_welcome2);
...
void wtlcd_welcome2()
{
	...
	MENU_ITEM_MULTI(gcode, MMSG_WELCOME_CONINUE[wtvar_language], PSTR("M104 T0 S210\r\nM701 T0\r\nM104 S0\r\nW102"));
	MENU_ITEM_MULTI(function, MMSG_WELCOME_SKIP[wtvar_language], wtlcd_quitwelcome);
	END_MENU();
...
void wtlcd_welcome3()
{
	...
	MENU_ITEM_MULTI(function, MMSG_WELCOME_CONINUE[wtvar_language], wtlcd_goto_welcome4);
	MENU_ITEM_MULTI(function, MMSG_WELCOME_SKIP[wtvar_language], wtlcd_quitwelcome);
	END_MENU();
}
...
void wtlcd_welcome4()
{
	...
	MENU_ITEM_MULTI(submenu, MMSG_CARD_MENU[wtvar_language], lcd_sdcard_menu);
	MENU_ITEM_MULTI(function, MMSG_WELCOME_SKIP[wtvar_language], wtlcd_quitwelcome);
	END_MENU();
}

extern char uploadFilename[FILENAME_LENGTH];
void wtlocd_UploadingScreen()
{
	START_SCREEN();
	STATIC_ITEM(" ", false, false);
	STATIC_ITEM_MULTI(MMSG_UPLOADING_1[wtvar_language], true, false);
	wt_single_line_message(3, uploadFilename);
	END_SCREEN();
}

void wtlcd_ShowUploadingScreen()
{
	menu_action_submenu(wtlocd_UploadingScreen);
}

void wtlocd_UploadFailScreen()
{
	START_SCREEN();
	STATIC_ITEM(" ", false, false);
	STATIC_ITEM_MULTI(MMSG_UPLOADING_FAIL[wtvar_language], true, false);
	END_SCREEN();
}

void wtlcd_ShowUploadFailScreen()
{
	menu_action_submenu(wtlocd_UploadFailScreen);
}
...
void wt_zoffset_menu()
{
	if (use_click())
	{
		zprobe_zoffset += current_position[Z_AXIS] - zoffset_beginZpos;
		(void)settings.save();
	...
	if (lcdDrawUpdate) 
	{
		//const float pos = NATIVE_TO_LOGICAL(current_position[Z_AXIS], Z_AXIS) - zoffset_beginZpos + zprobe_zoffset;
		const float pos = current_position[Z_AXIS] - zoffset_beginZpos + zprobe_zoffset;
...
void wt_zoffset_prepare_function()
{
	...
	enqueue_and_echo_commands_P(PSTR("G1 Z150 F500"));
	enqueue_and_echo_commands_P(PSTR("G28 Z"));
	enqueue_and_echo_commands_P(PSTR("G28 X Y F2000"));
	enqueue_and_echo_commands_P(PSTR("G29"));
	enqueue_and_echo_commands_P(PSTR("G90"));
	enqueue_and_echo_commands_P(PSTR("G1 X100 Y75 F2000"));
	enqueue_and_echo_commands_P(PSTR("G1 Z0 F100"));

	lcd_goto_screen(wt_zoffset_prepare_menu);
}

void wt_debug_sensor_menu()
{
	if (use_click()) { return lcd_goto_previous_menu(); }

	lcdDrawUpdate = LCDVIEW_CLEAR_CALL_REDRAW;

	_drawmenu_setting_edit_generic(false, 0, PSTR(MSG_X_MAX), READ(X_MAX_PIN) ? PSTR(MSG_DEBUG_SENSOR_ON) : PSTR(MSG_DEBUG_SENSOR_OFF), true);
	_drawmenu_setting_edit_generic(false, 1, PSTR(MSG_Y_MAX), READ(Y_MAX_PIN) ? PSTR(MSG_DEBUG_SENSOR_ON) : PSTR(MSG_DEBUG_SENSOR_OFF), true);
	_drawmenu_setting_edit_generic(false, 2, PSTR(MSG_Z_MAX), READ(Z_MAX_PIN) ? PSTR(MSG_DEBUG_SENSOR_ON) : PSTR(MSG_DEBUG_SENSOR_OFF), true);
#ifdef EnableRunoutAndDoor
	_drawmenu_setting_edit_generic(false, 3, PSTR(MSG_FILAMENT_RUNOUT_SENSOR), READ(FIL_RUNOUT_PIN) ? PSTR(MSG_DEBUG_SENSOR_OFF) : PSTR(MSG_DEBUG_SENSOR_ON), true);
	_drawmenu_setting_edit_generic(false, 4, PSTR(MSG_DEBUG_SENSOR_DOOR), READ(DOOROPEN_PIN) ? PSTR(MSG_DEBUG_SENSOR_OFF) : PSTR(MSG_DEBUG_SENSOR_ON), true);
#endif
}

void wt_filament_setting_on()
{
#ifdef EnableRunoutAndDoor
...
{
	START_MENU();
	STATIC_ITEM_MULTI(MMSG_FILAMENT_SENSOR[wtvar_language], true, false);

	MENU_ITEM_MULTI(function, MMSG_ON[wtvar_language], wt_filament_setting_on);
	MENU_ITEM_MULTI(function, MMSG_OFF[wtvar_language], wt_filament_setting_off);


	END_MENU();
}

void wt_door_setting_on()
{
	door_enable = 1;
	(void)settings.save();
	lcd_return_to_status();
...
{
	START_MENU();
	STATIC_ITEM_MULTI(MMSG_DOOR_SENSOR[wtvar_language], true, false);

	MENU_ITEM_MULTI(function, MMSG_ON[wtvar_language], wt_door_setting_on);
	MENU_ITEM_MULTI(function, MMSG_OFF[wtvar_language], wt_door_setting_off);
...
{
	START_SCREEN();
	STATIC_ITEM_MULTI(MMSG_NOTICE_MOVING[wtvar_language], false, false);
	END_SCREEN();
...
void wt_levelbed_begin()
{
	...
	if (wtvar_Z_MAX_POS == 170)
		enqueue_and_echo_commands_P(PSTR("G92 X200 Y150 Z170"));
	else
		enqueue_and_echo_commands_P(PSTR("G92 X200 Y150 Z165"));
	enqueue_and_echo_commands_P((PSTR("G1 Z5 F500")));
	enqueue_and_echo_commands_P((PSTR("G1 X175 Y148 F3000")));
	enqueue_and_echo_commands_P((PSTR("G1 Z0 F500")));

	lcd_goto_screen(wt_levelbed_moving);
}
...
void wt_load_sdcard()
{
	wtvar_sdcard_filecount = card.getnrfilenames();
	if (wtvar_sdcard_filecount % 6 > 0)
		wtvar_sdcard_pagecount = wtvar_sdcard_filecount / 6 + 1;
	else
		wtvar_sdcard_pagecount = wtvar_sdcard_filecount / 6;
...
void wt_sdcard_showpage()
{
	...
	if (wtvar_sdcard_filecount > 0 && wtvar_sdcard_pageid < wtvar_sdcard_pagecount)
	{
		...
		for (i = wtvar_sdcard_pageid * SDCARD_MENU_PAGE_ITEMS; i < endpos;  i++)
		{
			...
			strcpy((char*)&wtvar_sdcard_filelist[i - wtvar_sdcard_pageid * SDCARD_MENU_PAGE_ITEMS], card.longest_filename());
		}
	}
	else
	{
...
{
	if (door_enable == 1)
		lcd_goto_screen(wt_door_setting_menu, 1);
	else
		lcd_goto_screen(wt_door_setting_menu, 2);
}

void wt_Load_filament_setting_menu()
{
	if (wtvar_enablefilamentruncout == 1)
		lcd_goto_screen(wt_filament_setting_menu, 1);
	else
		lcd_goto_screen(wt_filament_setting_menu, 2);
}
...
void wtlcd_ChoiceZMaxMenu()
{
	...
	MENU_ITEM_MULTI(function, MSG_Z_MAX_170, wtlcd_zmax_170);
	MENU_ITEM_MULTI(function, MSG_Z_MAX_165, wtlcd_zmax_165);
...
void wtlcd_LoadZMaxMenu()
{
	...
	if (wtvar_Z_MAX_POS == 170)
		lcd_goto_screen(wtlcd_ChoiceZMaxMenu, 1);
	else
		lcd_goto_screen(wtlcd_ChoiceZMaxMenu, 2);
}
